<?php
  ...
  if(isset($_POST['cimage']))
    {
      ...
      $id=$_POST['cid'];
      $query=mysqli_query($con,"select image from supplier where cid='$id'");
  ...
  if(isset($_POST['salecart']))
    ...
    if(isset($_POST["confirmed"]))
    {
      ...
      $query=mysqli_query($con,"select max(order_id) from work ");
      while($row=mysqli_fetch_array($query))
        $order_id=$row[0];
      ...
      $query=mysqli_query($con,"select * from employer where cid='$pid'");
      while($row=mysqli_fetch_array($query))
        $name=$row['name'];
      ...
      while($row=mysqli_fetch_array($query))
        {
            ...
            $qt=$row['qt'];
            $extra=$row['extra'];
            $query2=mysqli_query($con,"insert into work(date,order_id,detail,name,quantity,price,total,p_id,payment,remaining,extra)values('$date','$order_id','$detail','$name','$qt','$price','$total','$pid','$payment','$remaining','$extra') ");
            if($query2)
            {
              $status=true;
            }
        ...
        if($status)
          { 
           ...
           $query=mysqli_query($con,"update employer set balance='$remaining' where cid='$pid'");
 ...
 if(isset($_POST['lim']))
    {
      if(isset($_POST["limit"], $_POST["start"]))
      {
            ...
            $query = mysqli_query($con, "SELECT Distinct order_id FROM work ORDER BY id DESC LIMIT ".$_POST["start"].", ".$_POST["limit"]."");
          ...
          {$sr=1;
            ...
            $payment;     
          $id=$row['order_id'];
          $query2 = mysqli_query($con, "SELECT * from work where order_id='$id'");
    ...
    if(isset($_POST['dltd']))
    {
      echo "Reached here";
      $id=$_POST['oid'];
      $query=mysqli_query($con,"delete from work where order_id='$id'  ");
      if($query)
  ...
  if(isset($_POST['resetsale']))
  {
	...
	{	
		...
		$query=mysqli_query($con,"select * from work where date='$formatdate'");
		 if(mysqli_num_rows($query)>0)
        {
