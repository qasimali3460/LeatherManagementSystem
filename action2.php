<?php
  ...
  if(isset($_POST['salecart']))
    ...
    if(isset($_POST["confirmed"]))
    {
      ...
      $query=mysqli_query($con,"select * from supplier where cid='$pid'");
      while($row=mysqli_fetch_array($query))
        $name=$row['name'];
      ...
      while($row=mysqli_fetch_array($query))
        {
            ...
            $qt=$row['qt'];
            $extra=$row['extra'];
            $query2=mysqli_query($con,"insert into purchase(date,order_id,detail,name,quantity,price,total,p_id,payment,remaining,extra)values('$date','$order_id','$detail','$name','$qt','$price','$total','$pid','$payment','$remaining','$extra') ");
            if($query2)
            {
              $status=true;
            }
        ...
        if($status)
          { 
           ...
           $query=mysqli_query($con,"update supplier set balance='$remaining' where cid='$pid'");
 ...
 if(isset($_POST['lim']))
    {
      if(isset($_POST["limit"], $_POST["start"]))
      {
            ...
            $query = mysqli_query($con, "SELECT Distinct order_id FROM purchase ORDER BY id DESC LIMIT ".$_POST["start"].", ".$_POST["limit"]."");
          ...
          {$sr=1;
            ...
            $payment;     
          $id=$row['order_id'];
          $query2 = mysqli_query($con, "SELECT * from purchase where order_id='$id'");
    ...
    if(isset($_POST['dltd']))
    {
      echo "Reached here";
      $id=$_POST['oid'];
      $query=mysqli_query($con,"delete from purchase where order_id='$id'  ");
      if($query)
  ...
  if(isset($_POST['resetsale']))
  {
	...
	{	
		...
		$query=mysqli_query($con,"select * from purchase where date='$formatdate'");
		 if(mysqli_num_rows($query)>0)
        {
